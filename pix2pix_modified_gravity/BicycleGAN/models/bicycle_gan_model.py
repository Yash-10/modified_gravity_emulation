...
##################### Added by me #####################
def interpolate(z1, z2, scale_factor=1.0):  # z1 is the encoded vector for F6 and z2 is for F4.
    towards_f6_from_f4 = z1 - z2
    towards_f6_from_f4_normed = towards_f6_from_f4 / torch.linalg.norm(towards_f6_from_f4)

    return z1 + scale_factor * towards_f6_from_f4_normed  # This vector will be along F4 --> F6, startingt from F6.
#######################################################
...
class BiCycleGANModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        # For cVAE-GAN, use below line instead.
        ...
        if opt.isTrain:
            ...
            self.criterionFFL = FFL(loss_weight=100.0, alpha=1.0, ave_spectrum=True, batch_matrix=True, log_matrix=True, patch_factor=2)
    ...
    def set_input(self, input, input2=None, latent_interpolate=False):
        AtoB = self.opt.direction == 'AtoB'
        self.real_A = input['A' if AtoB else 'B'].to(self.device)
        self.real_B = input['B' if AtoB else 'A'].to(self.device)
        if latent_interpolate and input2 is not None:
    ...
    def test(self, z0=None, encode=False, latent_interpolate=False, scale_factor=1.0):
        with torch.no_grad():
            if encode:  # use encoded z
                z0, _ = self.netE(self.real_B)
                if latent_interpolate:
                    z02, _ = self.netE(self.real_B2)
            if z0 is None:
                z0 = self.get_z_random(self.real_A.size(0), self.opt.nz)
            self.fake_B = self.netG(self.real_A, z0)
            if latent_interpolate:
                zrA, _ = self.netE(self.real_A)
                zrB, _ = self.netE(self.real_B)
                zfB, _ = self.netE(self.fake_B)
                zrB2, _ = self.netE(self.real_B2)
                #z_final = interpolate(z0, z02, scale_factor=scale_factor)
                #z_final = self.netE(self.real_B - self.real_B2)[0] + zrB2 
                #z_final = zrA + (zrB2 - zrA) / 2
                z_final = zrA
                self.fake_B2 = self.netG(self.real_A, z_final)
                zfB2, _ = self.netE(self.fake_B2)
            if latent_interpolate:
                return self.real_A, self.fake_B, self.real_B, self.fake_B2, zrA, zrB, zfB, zrB2, zfB2
            else:
    ...
    def backward_EG(self):
        ...
        if self.opt.lambda_L1 > 0.0:
            ...
            #self.loss_G_L1 = self.opt.lambda_L1 * (0.005 * self.criterionPerceptual(self.fake_B_encoded, self.real_B_encoded) + 50 * self.criterionFFL(self.fake_B_encoded, self.real_B_encoded))
            self.loss_G_L1 = self.opt.lambda_L1 * 0.5 * (self.criterionL1(self.fake_B_encoded, self.real_B_encoded) + self.criterionFFL(self.fake_B_encoded, self.real_B_encoded))  # Multiply by 0.5 to make the loss range same of only L1 in the original implementation.
        else:
            self.loss_G_L1 = 0.0
