...
#for epoch_num in [110]:
    ...
    for i in range(0, len(result_imgs), 3):
        ...
        test_curr = np.load(
                os.path.join('/cosma5/data/durham/dc-gond1/official_pix2pix_data_F5n1_GR/val', f'{result_imgs[i+1].split("/")[-1].split("_real")[0]}.npy')
        )
    ...
    chisquare_ip_gt_median, chisquare_gen_gt_median, chisquare_ip_gt, chisquare_gen_gt = driver(val_gen, val_ip, val_gt, vel_field=False, name=name, val_or_test=0, epoch_num=epoch_num,field_name='F5_den')
