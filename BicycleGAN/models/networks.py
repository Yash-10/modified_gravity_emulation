...
class D_NLayersMulti(nn.Module):
    ...
    def get_layers(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d):
        ...
        for n in range(1, n_layers):
            ...
            if n == 1:
                sequence += [
                        SpectralNorm2d(nn.Conv2d(ndf * nf_mult_prev, ndf * nf_mult,
                                  kernel_size=kw, stride=2, padding=padw)),
                        norm_layer(ndf * nf_mult),
                        nn.LeakyReLU(0.2, True),
                        ChannelAttention(ndf * nf_mult),
                        SpatialAttention()
                    ]
            else:
                sequence += [
        ...
        sequence += [SpectralNorm2d(nn.Conv2d(ndf * nf_mult, 1,
                               kernel_size=kw, stride=1, padding=padw))]
...
def upsampleLayer(inplanes, outplanes, upsample='basic', padding_type='zero'):
    # padding_type = 'zero'
    if upsample == 'basic':
        upconv = [SpectralNorm2d(nn.ConvTranspose2d(
...
class UnetBlock(nn.Module):
    def __init__(self, input_nc, outer_nc, inner_nc,
        ...
        downconv += [SpectralNorm2d(nn.Conv2d(input_nc, inner_nc,
        ...
        if outermost:
            ...
            down = downconv
            up = [uprelu] + [ChannelAttention(inner_nc * 2)] + [SpatialAttention()] + upconv + [nn.Tanh()]
            model = down + [submodule] + up
        elif innermost:
            upconv = upsampleLayer(
                inner_nc, outer_nc, upsample=upsample, padding_type=padding_type)
            down = [downrelu] + [ChannelAttention(input_nc)] + [SpatialAttention()] + downconv
            up = [uprelu] + [ChannelAttention(inner_nc)] + [SpatialAttention()] + upconv
            ...
            if downnorm is not None:
                down += [downnorm]
            up = [uprelu] + [ChannelAttention(inner_nc * 2)] + [SpatialAttention()] + upconv
            if upnorm is not None:
                up += [upnorm]
...
def convMeanpool(inplanes, outplanes):
    sequence = []
    sequence += [SpectralNorm2d(conv3x3(inplanes, outplanes))]
    sequence += [nn.AvgPool2d(kernel_size=2, stride=2)]
    return nn.Sequential(*sequence)
...
class BasicBlock(nn.Module):
    def __init__(self, inplanes, outplanes, norm_layer=None, nl_layer=None, attention=False):
        ...
        layers += [nl_layer()]
        if attention:
            layers += [ChannelAttention(inplanes)]
            layers += [SpatialAttention()]
        layers += [SpectralNorm2d(conv3x3(inplanes, inplanes))]
        if norm_layer is not None:
            layers += [norm_layer(inplanes)]
        layers += [nl_layer()]
        if attention:
    ...
    def __init__(self, input_nc=3, output_nc=1, ndf=64, n_blocks=4,
        ...
        for n in range(1, n_blocks):
            ...
            conv_layers += [BasicBlock(input_ndf,
                                               output_ndf, norm_layer, nl_layer, attention=True if n == 1 else False)] 
...
class G_Unet_add_all(nn.Module):
    def __init__(self, input_nc, output_nc, nz, num_downs, ngf=64,
        ...
        unet_block = UnetBlock_with_z(
            ngf, ngf, ngf * 2, nz, unet_block, norm_layer=norm_layer, nl_layer=nl_layer, upsample=upsample, attention_up=True)
...
class UnetBlock_with_z(nn.Module):
    def __init__(self, input_nc, outer_nc, inner_nc, nz=0,
                 submodule=None, outermost=False, innermost=False,
                 norm_layer=None, nl_layer=None, use_dropout=False, upsample='basic', padding_type='zero', attention_up=False):
