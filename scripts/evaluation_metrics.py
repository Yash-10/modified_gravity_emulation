...
#     plt.show()
    ...
    if vel_field:
        chisquare_ip_gt_cumulant = ps_metric(ips_cumulant_y, gts_cumulant_y).mean()
        chisquare_gen_gt_cumulant = ps_metric(gens_cumulant_y, gts_cumulant_y).mean()
        chisquare_ip_gt_cumulant_median = ps_metric(ips_cumulant_y_median, gts_cumulant_y_median).mean()
        chisquare_gen_gt_cumulant_median = ps_metric(gens_cumulant_y_median, gts_cumulant_y_median).mean()
    else:
        chisquare_ip_gt_cumulant = ps_metric(ips_cumulant_y, gts_cumulant_y).mean()
        chisquare_gen_gt_cumulant = ps_metric(gens_cumulant_y, gts_cumulant_y).mean()
        chisquare_ip_gt_cumulant_median = ps_metric(ips_cumulant_y_median, gts_cumulant_y_median).mean()
    ...
    gens_cumulant_x3, gens_cumulant_y3, _, gens_cumulant_y_std3, gens_cumulant_y_median3, gens_cumulant_y_iqr3, gens_cumulant_x_median3, _ = cumulant_overall(gens, n=3, vel_field=vel_field)
    ips_cumulant_x3, ips_cumulant_y3, _, ips_cumulant_y_std3, ips_cumulant_y_median3, ips_cumulant_y_iqr3, ips_cumulant_x_median3, _ = cumulant_overall(ips, n=3, vel_field=vel_field)
    gts_cumulant_x3, gts_cumulant_y3, _, gts_cumulant_y_std3, gts_cumulant_y_median3, gts_cumulant_y_iqr3, gts_cumulant_x_median3, gts_cumulant_x_iqr3 = cumulant_overall(gts, n=3, vel_field=vel_field)

    print("Distance for cumulants (n = 3).....")
    if vel_field:
        chisquare_ip_gt_cumulant = ps_metric(ips_cumulant_y3, gts_cumulant_y3).mean()
        chisquare_gen_gt_cumulant = ps_metric(gens_cumulant_y3, gts_cumulant_y3).mean()
        chisquare_ip_gt_cumulant_median = ps_metric(ips_cumulant_y_median3, gts_cumulant_y_median3).mean()
        chisquare_gen_gt_cumulant_median = ps_metric(gens_cumulant_y_median3, gts_cumulant_y_median3).mean()
    else:
    ...
    gens_cumulant_x4, gens_cumulant_y4, _, gens_cumulant_y_std4, gens_cumulant_y_median4, gens_cumulant_y_iqr4, gens_cumulant_x_median4, _ = cumulant_overall(gens, n=4, vel_field=vel_field)
    ips_cumulant_x4, ips_cumulant_y4, _, ips_cumulant_y_std4, ips_cumulant_y_median4, ips_cumulant_y_iqr4, ips_cumulant_x_median4, _ = cumulant_overall(ips, n=4, vel_field=vel_field)
    gts_cumulant_x4, gts_cumulant_y4, _, gts_cumulant_y_std4, gts_cumulant_y_median4, gts_cumulant_y_iqr4, gts_cumulant_x_median4, gts_cumulant_x_iqr4 = cumulant_overall(gts, n=4, vel_field=vel_field)

    print("Distance for cumulants (n = 4).....")
    if vel_field:
        chisquare_ip_gt_cumulant = ps_metric(ips_cumulant_y4, gts_cumulant_y4).mean()
        chisquare_gen_gt_cumulant = ps_metric(gens_cumulant_y4, gts_cumulant_y4).mean()
        chisquare_ip_gt_cumulant_median = ps_metric(ips_cumulant_y_median4, gts_cumulant_y_median4).mean()
        chisquare_gen_gt_cumulant_median = ps_metric(gens_cumulant_y_median4, gts_cumulant_y_median4).mean()
    else:
    ...
    # 3. PIXEL DISTANCE
    print('y_real PDF')
    x_real, y_real, _, y_real_std, y_real_median, y_real_iqr, x_real_median, x_real_iqr = pixel_pdf(gts, vel_field=vel_field)
    print('y_fake PDF')
    x_fake, y_fake, _, y_fake_std, y_fake_median, y_fake_iqr, _, _ = pixel_pdf(gens, vel_field=vel_field)
    print('y_ip PDF')
